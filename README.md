...
# Paragon MCP Server (Serverless)

This is an **experimental serverless** implementation for Model Context Protocol (MCP) that integrates with [ActionKit](https://useparagon.com/actionkit), an API by Paragon that provides access to prebuilt actions for 130+ integrations to your users' SaaS applications.

For a self-hostable **server** implementation, please visit the official [Paragon MCP](https://github.com/useparagon/paragon-mcp)
...
- `GET /mcp`: Establishes Streamable HTTP connection for MCP communication
- `POST /mcp`: Handles MCP message processing
- `GET /setup`: Handles integration setup flow

### Authorization

The `GET /mcp` endpoint (base URL for the MCP using the Steamable HTTP transport) accepts an `Authorization` header with a Paragon User Token as the Bearer token.
